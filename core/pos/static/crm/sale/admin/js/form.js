                ...
                dni: {
                    validators: {
                        ...
                        remote: {
                            ...
                            data: function () {
                                return {
                                    parameter: frmClient.querySelector('[name="dni"]').value,
                                    pattern: 'dni',
                ...
                mobile: {
                    validators: {
                        ...
                        remote: {
                            ...
                            data: function () {
                                return {
                                    parameter: frmClient.querySelector('[name="mobile"]').value,
                                    pattern: 'mobile',
                                    action: 'validate_client'
                                };
                            },
                ...
                email: {
                    validators: {
                        ...
                        remote: {
                            ...
                            data: function () {
                                return {
                                    parameter: frmClient.querySelector('[name="email"]').value,
                                    pattern: 'email',
                                    action: 'validate_client'
                                };
                            },
...
$(function () {
    ...
    $('#tblProducts tbody')
        ...
        .on('change', 'input[name="cant"]', function () {
            ...
            $('td:eq(-4)', tblProducts.row(tr.row).node()).html('Q' +sale.details.products[tr.row].subtotal.toFixed(2));
            $('td:last', tblProducts.row(tr.row).node()).html('Q' +sale.details.products[tr.row].total.toFixed(2));
        })
        .on('change', 'input[name="dscto_unitary"]', function () {
            var tr = tblProducts.cell($(this).closest('td, li')).index();
            sale.details.products[tr.row].dscto = parseFloat($(this).val());
            sale.calculateInvoice();
            $('td:eq(-2)', tblProducts.row(tr.row).node()).html('Q' +sale.details.products[tr.row].total_dscto.toFixed(2));
            $('td:last', tblProducts.row(tr.row).node()).html('Q' +sale.details.products[tr.row].total.toFixed(2));
        })
